...
function parsing(){
	...
	//		document.write(crule[ccnt].length+"<br>");
			for(j=0;j<crule[ccnt].length;j++){
				if(crule[ccnt][j]=='s' || crule[ccnt][j]=='b' || crule[ccnt][j]=='u' || crule[ccnt][j]=='d'|| crule[ccnt][j]=='r'|| crule[ccnt][j]=='l' || crule[ccnt][j]=='L' || crule[ccnt][j]=='R'){
...
//					document.write(rcode);
		...
		//	document.write(lcode[i][2]+"<br>");
			...
			//					document.write(mcode.length+"<br>");
								var dff=[];
								dff = dff + mcode;
							//	document.write(dff + "<br>");
								var df = [];
								df=cletter[k];
								var df2 = []; df2=crule[k];
								
								var dfdf = dff.replace(new RegExp(df,'g'),df2);
			//					document.write(dfdf+"<br>");
